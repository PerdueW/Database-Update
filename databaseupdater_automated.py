...
def insertData():
        root.folderpath = filedialog.askdirectory(initialdir = "Desktop",title = "Select Folder to Process")
        print(root.folderpath)
        folderpath = os.listdir(root.folderpath)
        if len(folderpath) == 0:
                print('Folder is empty. Choose another')
        else:
                ...
                for file in folderpath:
                        ...
                        #else:
                        #    pass
                        pathAct.config(text=file)
                        insert = open(root.folderpath + '/' + file, 'r')
